Python DB-Mapping Fragment Accumulator
...
# Overall Plan:
- python application
- input = two db~schema text files
- Produce a SQL-schema to Mongo-"schema" map json
...
- formats of input
- types of output (file output, in-state output, etc.)
...
#### Design Choices: Semi-Modular Iteration Through SQL-Schema

This activity has aspects of an arbitrary dummy exercise, but can be made and discussed to elucidate real-world factors.

The plan here will be to chop one schema into small-enough fragments (one table at a time), and compare that to the entire other schema (see discussion below about contextual analysis and collision prevention).
...
#### Planning for Real World Factors
...
(For a similar example using various API options, see: https://github.com/stemnetbenchmarks/social_story_and_cookbook_puzzles)


Note: input structuring (pydantic for MVP-1)
...
This format, roughly: see assignment doc for exact details
```
  "mapping_version": "1.0",
...
#### Retry Policy:
For MVP-1, when hitting a cloud LLM api there will be a basic retry N times (e.g. 10) before hard error. It is possible to include error/issue data in the retry, but for MVP-1 retry is fine. E.g. Issues with small mistral models were fundamental inability-to-do-task-issues, not issues that would have been helped by a tip. Larger models occasionally made different random mistakes, not repeat mistakes that would have been helped by a tip.



## Part 3. Produce Final Json
- final checks & processing
- output map_json report: save file, terminal print,
- output log report
...
The assignment-definition does not mention join-derived fields (such as employee-department), possibly with the assumption that these will be handled separately. The strictly-defined output format cannot express join-derived-fields. A tables[] entry contains one destination_collection, and each destination_field is a dot-path within that collection.
...
An attempt will be made to include useful information about these fields in a separate meta-data report.



### Versions: Python & Rust
...
Note: a Colab Version can be easily adapted from the .py version, for ease of testing and sharing.


2. Rust & Levinshtein Distance et al
